      ...
      <section className="workspace">
        ...
        <div className="content-panel">
          ...
          {view === "kosakata" && <div className="vocab-view">
            ...
            {view === "kosakata" && <section className="lexicon"><div><p>REFERENSI KOSAKATA JLPT</p><h3>{lexicon.length.toLocaleString("id-ID")} kata N5–N4</h3><span>Seluruh arti ditampilkan dalam bahasa Indonesia. Tekan kartu untuk mendengar pelafalan Jepangnya.</span></div><div className="lexicon-tools"><input aria-label="Cari kamus" value={lexiconQuery} onChange={(event)=>setLexiconQuery(event.target.value)} placeholder="Cari Jepang, cara baca, atau arti Indonesia…"/><select value={lexiconLevel} onChange={(event)=>setLexiconLevel(event.target.value as typeof lexiconLevel)}><option>Semua</option><option>N5</option><option>N4</option></select></div><div className="lexicon-grid">{filteredLexicon.map((item)=><button key={item.id} className={playingAudioKey===audioKey(item.word)&&audioStatus==="playing"?"playing":""} onClick={()=>speakJapanese(item.word)}><b>{item.word}</b><span>{item.reading}</span><small>{item.level} · {item.meaning}</small></button>)}</div><small>Data referensi: open-anki-jlpt-decks (MIT), dengan arti Indonesia. Maksimal 80 hasil ditampilkan agar tetap ringan.</small></section>}
          ...
          {view === "percakapan" && <div className="conversation-view">
            <div className="section-title"><div><p>会話 · KAIWA ANIMASI</p><h2>Belajar lewat percakapan kartun</h2></div><span>Teks Jepang · terjemahan Indonesia · kosakata N5–N4.</span></div>
            <div className="kaiwa-source-note"><span>映像</span><div><b>Tonton teks Jepang → cek terjemahan Indonesia → ulangi → pakai sendiri.</b><p>Setiap video membagi pembelajaran menjadi percakapan bertulisan Jepang, terjemahan Indonesia, lalu ulasan kosakata. Coba pahami situasinya sebelum melihat terjemahan.</p></div><a href={activeKaiwa.source} target="_blank" rel="noreferrer">Buka di YouTube ↗</a></div>
